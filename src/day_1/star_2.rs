const INPUT: &str = "rhqrpdxsqhgxzknr2foursnrcfthree
2bmckl
four95qvkvveight5
...
fn parse_number_from_string(input: &str) -> String {
    let found = match input.to_lowercase().as_str() {
        "one" => Some("1"),
        "two" => Some("2"),
        "three" => Some("3"),
        "four" => Some("4"),
...
fn find_first_occurrence(table: &[&str], text: &str) -> Option<String> {
    table
        .iter()
        .filter_map(|&lookup| text.find(lookup).map(|_| lookup))
        .min_by_key(|&lookup| text.find(lookup).unwrap_or(usize::MAX))
        .map(|s| s.to_string())
}

fn find_last_occurrence(table: &[&str], text: &str) -> Option<String> {
    table
        .iter()
        .filter_map(|&lookup| text.rfind(lookup).map(|_| lookup))
        .max_by_key(|&lookup| text.rfind(lookup).unwrap_or(0))
        .map(|s| s.to_string())
}

fn resolve_first_and_last(line: &str) -> String {
    let lookup = [
        "1", "2", "3", "4", "5", "6", "7", "8", "9", "one", "two", "three", "four", "five", "six",
        "seven", "eight", "nine",
    ];

    let first = find_first_occurrence(&lookup, line)
        .map(|v| parse_number_from_string(v.as_str()))
        .unwrap();

    let last = find_last_occurrence(&lookup, line)
        .map(|v| parse_number_from_string(v.as_str()))
        .unwrap();
...
pub fn run() {
    let results = INPUT.split('\n').map(resolve_first_and_last);

    println!(
        "{}",
        results.filter_map(|v| v.parse::<i32>().ok()).sum::<i32>()
    );
}
