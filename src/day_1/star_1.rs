...
fn find_first_digit(text: &str) -> Option<String> {
    text.chars().find(|c| c.is_digit(10)).map(|c| c.to_string())
}

fn resolve_first_and_last(line: &str) -> String {
    let first = find_first_digit(line).unwrap_or_default();
    let last = find_first_digit(&line.chars().rev().collect::<String>()).unwrap_or_default();
