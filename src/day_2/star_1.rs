...
fn is_game_possible(game: &(usize, (String, Vec<HashMap<String, i32>>))) -> bool {
    const RED_THRESHOLD: i32 = 12;
    const GREEN_THRESHOLD: i32 = 13;
    const BLUE_THRESHOLD: i32 = 14;

    game.1 .1.iter().all(|round| {
        *round.get("red").unwrap_or(&0) <= RED_THRESHOLD
            && *round.get("blue").unwrap_or(&0) <= BLUE_THRESHOLD
            && *round.get("green").unwrap_or(&0) <= GREEN_THRESHOLD
    })
}
