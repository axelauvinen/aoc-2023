...
const INPUT: &str = "Game 1: 4 blue, 7 red, 5 green; 3 blue, 4 red, 16 green; 3 red, 11 green
...
fn find_max_count_for_colors(rounds: Vec<HashMap<String, i32>>) -> HashMap<String, i32> {
    rounds.iter().fold(HashMap::new(), |mut max_counts, round| {
        for (color, &count) in round.iter() {
            max_counts
                .entry(color.clone())
...
fn parse_title(data: &str) -> (&str, &str) {
    data.split_once(':').map(|(t, d)| (t.trim(), d)).unwrap()
}

fn parse_group(group: &str) -> HashMap<String, i32> {
    group
        .split(',')
...
fn parse_item(item: &str) -> Option<(String, i32)> {
    let parts: Vec<&str> = item.split_whitespace().collect();
    match parts.as_slice() {
        [count, color] => Some(((*color).to_string(), count.parse().unwrap_or(0))),
        _ => None,
    }
}

fn parse_game_data(data: &str) -> (String, Vec<HashMap<String, i32>>) {
    let (title, data) = parse_title(data);
    let parsed_data = data.split(';').map(parse_group).collect();
    (title.to_string(), parsed_data)
}
...
pub fn run() {
    let sum: i32 = INPUT
        .split("\n")
        .map(parse_game_data)
        .map(|(_, rounds)| find_max_count_for_colors(rounds))
        .map(|max_counts| max_counts.values().product::<i32>())
        .sum();
